...
// ── Diagnostic Coach — ratios/spikes (verdict Insolvency retiré) ────────────
// L'Insolvency Index émettait un verdict d'intensité concurrent (« Banqueroute
// — Deload immédiat ») hors de l'arbitre, et sa calibration était cassée
// (banqueroute permanente pour tout utilisateur assidu). Un seul verdict
// d'intensité désormais : l'arbitre. calcInsolvencyIndex reste inerte (backlog :
// recalibration éventuelle → entrée de l'arbitre, jamais verdict).
function analyzeAthleteProfileWithInsolvency() {
  return typeof analyzeAthleteProfile === 'function' ? analyzeAthleteProfile() : [];
}
