...
async function syncFromCloud() { if (!supaClient) return false; try { const {data:{user}} = await supaClient.auth.getUser(); if (!user) return false; const {data, error} = await supaClient.from('sbd_profiles').select('data,updated_at').eq('user_id', user.id).maybeSingle(); if (error) throw error; if (data && data.data) { db = data.data; if (db.weeklyPlan && db.weeklyPlan.days) { db.weeklyPlan.days.forEach(function(day) { if (day.exercises) day.exercises = day.exercises.filter(function(e) { return !e.isPrehab; }); }); syncToCloud(true); } if (!db.reports) db.reports = []; if (!db.logs) db.logs = []; if (!db.user) db.user = {}; if (!db.user.targets) db.user.targets = { bench: 100, squat: 120, deadlift: 140 }; if (!db.social) db.social = {}; if (!db.bestPR) db.bestPR = { bench: 0, squat: 0, deadlift: 0 }; if (!db.gamification) db.gamification = {}; db.lastSync = data.updated_at ? new Date(data.updated_at).getTime() : Date.now(); localStorage.setItem(STORAGE_KEY, JSON.stringify(db)); refreshUI(); showToast('Données cloud chargées !'); return true; } else { showToast('Aucune donnée cloud trouvée'); return false; } } catch(e) { console.error('Cloud pull:', e); showToast('Erreur chargement cloud'); return false; } }
